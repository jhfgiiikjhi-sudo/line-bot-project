...
async function handleEvent(event) {
   ...
   // 9. REGISTER FLOW (CORE) - ปรับปรุงให้ Flow ต่อเนื่อง
    ...
    if (lower.includes("ข่าว")) {
        const newsList = await getLatestNews(2); // ดึงข่าวมาเตรียมไว้ 2 ข่าว
        if (newsList && newsList.length > 0) {
            let selectedNews = newsList[0]; // ค่าเริ่มต้นคือข่าวที่ 1 (ล่าสุด)
            let typeText = "ล่าสุด! 🔥";

            // ตรวจสอบว่าถ้า User ถามหาข่าวเก่า (เมื่อวาน, ก่อนหน้า, เก่า)
            if (lower.includes("เมื่อวาน") || lower.includes("ก่อนหน้า") || lower.includes("เก่า") || lower.includes("ย้อนหลัง")) {
                if (newsList.length > 1) {
                    selectedNews = newsList[1]; // เปลี่ยนไปหยิบข่าวที่ 2 แทน
