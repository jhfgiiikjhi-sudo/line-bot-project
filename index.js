...
if (user.step === "done") {
    try {
        ...
        // จัดการเรื่องวันที่ไว้ที่นี่
        const now = moment().tz("Asia/Bangkok");
        const dateStr = now.format("ddddที่ D MMMM YYYY เวลา HH:mm น.");
        ...
        const aiResponse = await openai.chat.completions.create({
            ...
            messages: [
                { 
                    role: "system", 
                    content: `คุณคือ "พี่บอท SPTC" ที่ปรึกษาผู้ใจดีและเป็นกันเองจากวิทยาลัยเทคโนโลยีสมุทรปราการ
                    ...
                    [บริบทเวลาและข่าวสาร]
                    - วันนี้คือ: ${dateStr} (ยึดถือว่าเป็นปี ค.ศ. 2026 เท่านั้น)
                    - ข่าวล่าสุด: ${global.latestNewsTitle || "ไม่มีข่าวใหม่ในขณะนี้"}
                    - ข้อมูลวิทยาลัย: ${JSON.stringify(collegeData)}

                    [แนวทางการตอบ]
                    1. แทนตัวเองว่า "พี่บอท" เสมอ
                    2. ตอบสุภาพ มีหางเสียง (ครับ) และเป็นกันเองกับน้องๆ
                    3. หากข้อมูลใน collegeData มีคำตอบ ให้ยึดตามนั้น
                    4. ถ้าไม่รู้จริงๆ ให้บอกน้องว่า "พี่บอทไม่แน่ใจข้อมูลส่วนนี้ รบกวนน้องติดต่อที่ห้องประชาสัมพันธ์วิทยาลัยโดยตรงนะครับ"` 
                },
