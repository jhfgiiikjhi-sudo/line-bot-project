...
function isStrictlyHumanName(text) {
  ...
  // 3. ดักการรูดแป้น (ฟหกด) - ปรับลดเหลือ 4 ตัว และเช็คสระ
  // ภาษาไทยปกติแทบไม่มีคำไหนที่มีพยัญชนะติดกัน 4 ตัวโดยไม่มีสระ (ยกเว้นมีตัวการันต์ หรือคำเฉพาะ)
...
// ตรวจชื่อคนจริง (ไทย / อังกฤษ) - ฉบับอัปเดตสระ
function isHumanName(text, min, max) {
  ...
  // ไทยต้องมีสระ (เพิ่ม ั และ ็ เพื่อรองรับชื่ออย่าง ตั้ม, เป็ด)
  if (/^[ก-ฮ]+$/.test(text) && !/[ะาิีึืุูเแโใไำั็]/.test(text))
    return false;
...
async function handleEvent(event) {
  ...
  if (msgType !== "normal") {
    // ลบการบวกแต้มตรงนี้ออก (บรรทัดเดิม: user.badCount = (user.badCount || 0) + 1;)
    ...
    if (hasBadWord(text)) {
      user.badCount = (user.badCount || 0) + 1; // ให้มาบวกแต้มตรงนี้ที่เดียว!
      saveUsers();
