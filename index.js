...
async function handleEvent(event) {
    ...
    if (lower.includes("ข่าว")) {
        ...
        if (newsList && newsList.length > 0) {
            ...
            if (lower.includes("เมื่อวาน") || lower.includes("ก่อนหน้า")) {
                if (newsList.length > 1) { selectedNews = newsList[1]; typeText = "เมื่อวาน/ก่อนหน้า 📰"; }
            }
    ...
    if (user.step === "done") {
        try {
            const dateStr = now.format("LLLL"); // เพิ่มประกาศตัวแปรที่ขาดหาย
            const aiResponse = await openai.chat.completions.create({
                ...
                messages: [
                    { 
                        ...
                        [แนวทางการตอบ]
                        1. แทนตัวเองว่า "พี่บอท" 2. ตอบสุภาพ มีหางเสียง 3. ถ้าไม่รู้ให้แนะนำติดต่อห้องประชาสัมพันธ์` 
                    },
