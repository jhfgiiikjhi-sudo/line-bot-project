...
async function handleEvent(event) {
  ...
  if (user.step === "ask_name") {
    ...
    // ❌ ไทยล้วนแต่ไม่มีพยัญชนะหลัก
    const thaiLetters = userText.match(/[ก-ฮ]/g) || [];
    const thaiVowels = userText.match(/[ะาิีึืุูเแโใไำ]/g) || [];
    if (thaiLetters.length > 0 && thaiVowels.length === 0) {
