...
const DEPARTMENTS = [
  ...
  "การบัญชี", "การตลาด", "เทคโนโลยีสารสนเทศ", 
...
// เพิ่มฟังก์ชันนี้ไว้ด้านบน (ใกล้ๆ กับ isHumanName เดิม)
function isStrictlyHumanName(text) {
  // 1. เช็คว่ามีตัวอักษรซ้ำกันเกิน 4 ตัวไหม (เช่น กกกกก)
  const repetitiveChars = /(.)\1{3,}/;
  if (repetitiveChars.test(text)) return false;

  // 2. เช็คว่าเป็นภาษาไทยหรืออังกฤษเท่านั้น (ไม่มีสัญลักษณ์/ตัวเลข)
  const nameRegex = /^[a-zA-Zก-๙\s]+$/;
  if (!nameRegex.test(text)) return false;

  // 3. ความยาวชื่อจริงควรอยู่ระหว่าง 3 - 30 ตัวอักษร
...
if (user.step === "ask_birthday") {
  ...
  user.step = "ask_department"; // เปลี่ยนจาก done เป็น ask_department
  saveUsers();
  return reply(event, `ต่อไปขอทราบ **สาขาวิชา** ของคุณครับ\nตัวอย่าง: ช่างยนต์, เทคโนโลยีสารสนเทศ\n(เลือกจากสาขาที่ SPTC เปิดสอนนะครับ)`);
}
