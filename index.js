...
async function handleEvent(event) {
   ...
   // --- ระบบเช็คข่าวสาร (Smart News) ---
    if (lower.includes("ข่าว")) {
        // ดึงข่าวมาเผื่อ 5 ข่าว เพื่อให้มีตัวเลือกสำรองเสมอ
        const newsList = await getLatestNews(5); 
        
        if (newsList && newsList.length > 0) {
            let selectedNews = newsList[0]; // ค่าเริ่มต้นคือข่าวล่าสุด
            let typeText = "ล่าสุด! 🔥";

            // ตรวจสอบ Keyword สำหรับข่าวเก่า/เมื่อวาน
            if (lower.includes("เมื่อวาน") || lower.includes("ก่อนหน้า") || lower.includes("เก่า") || lower.includes("ย้อนหลัง")) {
                if (newsList.length > 1) {
                    selectedNews = newsList[1]; // เลือกข่าวลำดับที่ 2
                    
                    // ปรับหัวข้อให้ตรงกับสิ่งที่ผู้ใช้พิมพ์
