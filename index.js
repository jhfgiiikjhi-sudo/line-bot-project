...
async function handleEvent(event) {
  ...
  // ===== 5. CHANGE COMMANDS (แก้ไขใหม่) =====
  if (lower.includes("เปลี่ยนชื่อเล่น")) {
    user.step = "ask_nickname_only"; // เปลี่ยนจาก ask_nickname เป็นแบบ _only
    saveUsers();
    return reply(event, "ได้เลยครับ 😊\nกรุณาพิมพ์ชื่อเล่นใหม่ได้เลย");
  }
  if (lower.includes("เปลี่ยนชื่อ")) {
    user.step = "ask_realname_only"; // เปลี่ยนเป็นแบบ _only
    saveUsers();
    return reply(event, "ได้เลยครับ 😊\nกรุณาพิมพ์ชื่อจริงใหม่ได้เลย");
  }
  if (lower.includes("เปลี่ยนอายุ")) {
    user.step = "ask_age_only"; // เปลี่ยนเป็นแบบ _only
    saveUsers();
