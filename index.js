...
async function handleEvent(event) {
   ...
   // 9. REGISTER FLOW (CORE) - ปรับปรุงให้ Flow ต่อเนื่อง
    ...
    if (user.step && user.step.startsWith("ask_realname")) {
        ...
        // ถ้าข้อมูลครบหมดแล้วจริงๆ ถึงจะให้หยุดที่ "done"
        ...
        // ถ้าข้อมูลยังไม่ครบ ให้ไหลไปถามชื่อเล่นต่อ
        user.step = "ask_nickname";
        await user.save();
        const prefix = lower.includes("เปลี่ยนชื่อ") ? `✅ อัปเดตชื่อจริงเรียบร้อยครับคุณ ${text}\n` : `ยินดีที่ได้รู้จักครับคุณ ${text} 😊\n`;
    ...
    if (lower.includes("ข่าว")) {
        // ดึงข่าวมาเผื่อ 5 ข่าว เพื่อให้มีตัวเลือกข่าวลำดับที่ 2 (index 1) เสมอ
        const newsList = await getLatestNews(5); 
        
        if (newsList && newsList.length > 0) {
            let selectedNews = newsList[0]; // ค่าเริ่มต้นคือข่าวลำดับที่ 1 (ล่าสุด)
            let typeText = "ล่าสุด! 🔥";

            // ตรวจสอบ Keyword สำหรับข่าวเก่า
            if (lower.includes("เมื่อวาน") || lower.includes("ก่อนหน้า") || lower.includes("เก่า") || lower.includes("ย้อนหลัง")) {
                if (newsList.length > 1) {
                    selectedNews = newsList[1]; // สลับไปใช้ข่าวลำดับที่ 2
                    typeText = "ข่าวก่อนหน้า 📰";
                } else {
                    // กรณีพบข่าวในหน้าเว็บเพียงข่าวเดียว
                    return reply(event, "📢 ตอนนี้พี่บอทพบข่าวล่าสุดเพียงข่าวเดียวครับ ยังไม่มีข่าวเก่าในระบบหน้าเว็บ");
                }
            }
            ...
            // กรณีดึงจากเว็บแบบ Real-time ไม่สำเร็จ ให้ใช้ข่าวล่าสุดที่เก็บในระบบ Global
