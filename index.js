...
function isTextSpam(text) {
    if (!text) return false;
    // ดักจับการพิมพ์สัญลักษณ์ซ้ำๆ หรือตัวอักษรเดียวซ้ำๆ
...
async function handleEvent(event) {
    ...
    // 7. คำสั่งพิเศษ (ปรับปรุงให้ฉลาดขึ้น)
    // --- แก้ไขบรรทัดที่ 438 ใน index.js ---
if (lower === "ยกเลิก" || lower === "เริ่มใหม่") {
    user.step = "ask_realname";
    // เพิ่มการล้างข้อมูลเดิมทิ้งตรงนี้ครับ เพื่อให้ระบบส่งไปขั้นตอนถัดไปต่อ
    ...
    await user.save();
    return reply(event, "🤖 รีเซ็ตระบบลงทะเบียนให้ใหม่แล้วครับ!\n\nกรุณาพิมพ์ **ชื่อจริง-นามสกุล** ของคุณเพื่อลงทะเบียนใหม่ครับ");
}
    }
